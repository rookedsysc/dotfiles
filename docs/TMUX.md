...
| 명령어 | 단축키 | 설명 | Zellij 대응 |
|--------|--------|------|-------------|
| `tmux new -s <session-name>` | - | 새 세션 생성 | `za <session-name>` |
| `tmux attach -t <session-name>` | - | 세션에 연결 | `za <session-name>` |
| `C-b d` | `Prefix + d` | 현재 세션에서 detach | `C-o` (zellij) |
| `C-b s` | `Prefix + s` | 세션 목록 보기 및 전환 | - |
| `C-b (` / `C-b )` | `Prefix + (` / `)` | 이전/다음 세션으로 전환 | - |
| `C-b $` | `Prefix + $` | 세션 이름 변경 | - |
| `tmux kill-session -t <name>` | - | 세션 삭제 | `zd` |

### 2. 윈도우 관리 (브랜치별 작업 공간)

| 명령어 | 단축키 | 설명 |
|--------|--------|------|
| `C-b c` | `Prefix + c` | 새 윈도우 생성 |
| `C-b ,` | `Prefix + ,` | 윈도우 이름 변경 |
| `C-b n` | `Prefix + n` | 다음 윈도우로 이동 |
| `C-b p` | `Prefix + p` | 이전 윈도우로 이동 |
| `C-b 0-9` | `Prefix + 숫자` | 특정 윈도우로 이동 |
| `C-b w` | `Prefix + w` | 윈도우 목록 보기 및 선택 |
| `C-b &` | `Prefix + &` | 현재 윈도우 종료 |
| `C-b l` | `Prefix + l` | 마지막 윈도우로 전환 |
